...
program compute_pi
  ...
  integer(kind=8) :: sum_pi_res
  real :: pi_approx, pi_true
  real :: x,y,r

  integer :: myRank, nbTask, ierr
  integer, dimension(MPI_STATUS_SIZE) :: status2
  ...
  call MPI_REDUCE(sum_pi, sum_pi_res, 1, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, ierr)

  call cpu_time(t2)

  if (myRank == 0) then 
     write(*,*) 'N=', N, ', sum_pi=',sum_pi_res
     pi_approx = (4.0*sum_pi_res)/N
